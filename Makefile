...
clean:
	...
	@rm -rfv $(BUILD_DIR)/* $(COVERAGE_DIR) coverage.xml

distclean: clean
	@echo "=== $(PROJECT_NAME) === [ distclean        ]: removing binaries and coverage file..."
	@rm -rf vendor/*/
...
compile-only: deps-only
	...
	@for b in $(BINS); do \
		...
		$(GO) build -ldflags "-X main.version=$(VERSION)" -o $(BUILD_DIR)/$$b $$BUILD_FILES ; \
	done
...
docker-build:
	@echo "=== $(PROJECT_NAME) === [ docker-build     ]: Building docker container..."
	@docker build -t $(MAINTAINER)/$(PROJECT_NAME):$(VERSION) -f build/Dockerfile .
...
.PHONY: all build clean distclean coverage document document-only document-deps fmt lint vet validate-deps validate-only validate compile-deps compile-only compile test-deps test-unit test-integration test-only test
